    ...
    <div class="form-horizontal">
            ...
            <div class="form-group form-group-flex mb-1">
                <div class="form-checkbox col-md-4 pt-6x wkeep wnowrap">
                    <label class="control control--radio">
                        <input type="radio" name="ticketType" value="freeSeat" v-on:click="typeChange('freeSeat')" v-model="typeTicketSetting" :disabled="statucControl[0].seat_class_kbn">{{ trans('events.S_FreeSeatTitle') }}
        ...
        <div class="" v-show="(typeTicketSetting == 'freeSeat') ? true : false">
            ...
            <div class="form-group-flex">
                <div class="txt-flex-mr">{{ trans('events.S_freeSeatQtyTitle') }}</div>
                    <div class="col-md-2">
                    ...
                    <div v-show="!ticketOnSiteShow" class="txt-flex-mr"> {{ trans('events.S_FreeSeatInf') }}</div>
            </div>
            </div>
            </div>
                    ...
                    <div class="box-header box-header-forms-gray">
                        ...
                        <div class="col-xs-12 ptb-8x form-group-flex">
                            <div class="col-xs-4 pl-x has-feedback">
                                <!--
                                <input type="text" class="form-control input-sm" v-validate="'required'" v-model="freeSeatTicketName" name="free-Ticket-Name" type="text" @blur="freeSeatNameChange()" :disabled="statucControl[0].seat_class_name" placeholder="{{ trans('events.S_seatTypePlaceholder') }}">
                                <span class="glyphicon glyphicon-remove-sign form-control-feedback" @click="freeSeatNameClear()"></span>
                                -->
                                <input type="text" class="form-control input-sm" maxlength="25" v-bind:style="{ borderColor: (freeticketErrorStatus)?'#e44e2d':'' }"  v-model="freeSeatTicketName" type="text" @change="cheackFreeSeatData()" @blur="freeSeatNameChange()" :disabled="statucControl[0].seat_class_name" placeholder="{{ trans('events.S_seatTypePlaceholder') }}">
                            </div>
                    ...
                    <div class="box-body table-responsive pt-x pl-70">
                        ...
                        <table id="freeTicket" class="table table-striped table-ticket">
                            <thead>
                                <tr>
                                    <th width="40px"></th>
                                    <th width="30%">{{ trans('events.S_TicketNameTitle') }}</th>
                                    <th width="20%">{{ trans('events.S_TicketPriceTitle') }}</th>
                                    <th>{{ trans('events.S_TicketEarlyBirdTitle') }} </th>
                                    <th>{{ trans('events.S_TicketNormalTitle') }}</th>
                                    <th  v-show="ticketOnSiteShow">{{ trans('events.S_TicketOnSiteTitle') }}</th>
                                    <th id="insertFreeSeat" class="text-right">
                                        <button class="btn btn-inverse btn-m" v-on:click="insertFreeSeat()" v-show="!statucControl[0].seat_class_name">{{ trans('events.S_TicketTypeInsertBtn') }}</button>
                                    </th>
                                </tr>
                            </thead>
                            <tbody>
                                    ...
                                    <tr v-if="(item.ticketStatus !== 'D')">
                                        ...
                                        <td>
                                            ...
                                            <!-- <div v-bind:class="{ 'select-error-tip': item.ticketNameErrorStatus }">  -->
                                                <select :id="'nomSelect-'+index" :value="item.ticketName" :data-index="index"  onchange="ticketSetting.nomSeatSelectChange(this)" class="form-control select2 select-create"  style="width: 100%;" :disabled="statucControl[0].ticket_class_name">
                                                        ...
                                                        <!--<option value="" class="not-select" disabled>{{ trans('events.S_ticketTypePlaceholder') }}</option>-->
                                                    <option value="" class="not-select" selected="true" disabled="disabled">{{ trans('events.S_ticketTypePlaceholder') }}</option>
                                                    <template  v-for="inf in selectOption">
                                                        <option v-if="inf.id !== 'nomSelect-'+index" :value="inf.value">@{{ inf.value }}</option>
                                                    </template>
                                                </select> 
                                                <span v-show="item.ticketNameErrorStatus" class="help is-danger"><i class="fas fa-exclamation-circle"></i> {{ trans('events.S_basicerrorMsn') }}</span>{{-- STS 2021/07/27 Task 38  --}}
                                            <!-- </div> -->
                                        </td>
                                        <td>
                                          <!--    <input name="text" type="number" min="0" class="form-control text-right free-ticket-price" v-bind:style="{ borderColor: (item.ticketErrorStatus)?'#e44e2d':'' }" :value="item.ticketPrice" @blur="inputPrice(index, $event)" :disabled="statucControl[0].price" placeholder="{{ trans('events.S_ticketPricePlaceholder') }}"> -->
                                            <!-- STS 2021/06/12: 小切手 4 つの特殊文字 (+)、(-)、(.)、(,) -->
                                            <input @paste="validateNumber" onkeypress="return event.charCode >= 48 && event.charCode <= 57" name="text" type="number" min="0" class="form-control text-right free-ticket-price" v-bind:style="{ borderColor: (item.ticketErrorStatus)?'#e44e2d':'' }" :value="item.ticketPrice" @blur="inputPrice(index, $event)" :disabled="statucControl[0].price" placeholder="{{ trans('events.S_ticketPricePlaceholder') }}">
                                        </td>
                                        <td>
                                        ...
                                        <td class="text-right">
                                            <button class="btn btn-danger btn-m" v-on:click="removeRow(index)" v-if="freeTicketDelBtn(item)">
                                                {{ trans('events.S_TicketTypeDeleteBtn') }}
                                            </button>
                                        </td>
                                    </tr>
                                </template>
            ...
            <div id="selectSeat" class="" v-show="(timeCourse.settingRadio !== 'spec') ? true : false">
                <div class="form-group form-group-flex mb-1">
                    ...
                    <div class="form-checkbox col-md-12 pt-6X">
                        <label class="control control--radio">
                            <input type="radio" name="ticketType" v-on:click="typeChange('selectSeat')" v-model="typeTicketSetting" value="selectSeat" :disabled="statucControl[0].seat_class_kbn">{{ trans('events.S_TicketSeatSelectTitle') }}
                            <div class="control__indicator"></div>
                        </label>
                    </div>
                    <!-- /.radiobox -->
                ...
                <div class="ml-3" v-show="(typeTicketSetting == 'selectSeat') ? true : false">
                    <div class="form-checkbox-groupbox-wt">
                        <div class="form-checkbox">
                            <label class="control control--checkbox">
                                <input type="checkbox" v-model="selection" :disabled="statucControl[0].base_kbn">{{ trans('events.S_TicketSeatSelfSelectionTitle') }}
                                <div class="control__indicator"></div>
                            </label>
                        </div>
                        <p class="pl-30 text-gray"> 
                            {{ trans('events.S_TicketSeatSelfSelectionDesc') }}
                        </p>
                    </div>
                </div>
        ...
        <div class="" v-show="(typeTicketSetting == 'selectSeat') ? true : false">
           <!-- sortable-wraper -->
            ...
            <template v-for="(item, index) in ticketSortIndex">
                    ...
                    <!-- 錯誤訊息提示 -->
                    {{-- <div class="ml-3" v-show="settingSeatData[item].seatErrorStatus || (!settingSeatData[item].seatName && settingSeatData[item].seatStatus !== 'D')">    --}}
                        <div class="" v-show="settingSeatData[item].seatErrorShowStatus || settingSeatData[item].seatErrorMsn "> {{-- STS 2021/07/27 Task 38 --}}
                        <div class="callout callout-txt-warning ">
                            <div class="icon">
                                <i class="fas fa-exclamation-circle fa-lg"></i> 
                            </div>
                            ...
                            <div class="box-header box-header-forms-gray">
                                ...
                                <div class="col-xs-12 ptb-8x form-group-flex">
                                    <div class="col-xs-5 pl-x has-feedback">
                                        ...
                                        <input :name="'Ticket-Name'+item" type="text" maxlength="25" v-bind:style="{ borderColor: (!settingSeatData[item].seatName)?'#e44e2d':'' }" class="form-control input-sm" :value="settingSeatData[item].seatName" @blur="settingSeatTitleChange(item, $event)" :disabled="statucControl[0].seat_class_name"  placeholder="{{ trans('events.S_seatTypePlaceholder') }}">
                                    </div>
                                    ...
                                    <div class="col-xs-4 form-checkbox wkeep wnowrap">
                                        <label class="control control--checkbox mtbl-1">
                                            <input type="checkbox" :id="'freeSeat_'+item" @click="settingSeatFreeCheackChange(item, $event, 'colorPick-'+item)"  v-bind:checked="settingSeatData[item].seatFree"  :disabled="freeSeatBtnDis(settingSeatData[item], item)">  {{ trans('events.S_freeSeatTitle') }} <small>[ ※ 券面に通し番号有 ]</small>
                                            <div class="control__indicator"></div>
                                        </label>
                                    </div>
                                    <div class="col-xs-2 form-checkbox wkeep">
                                        <label class="control control--checkbox mtbl-1" v-show="!settingSeatData[item].seatFree">
                                            <input :id="'nextSeat'+item" type="checkbox" @click="seatOnNext(item, $event)" v-bind:checked="settingSeatData[item].seatNextSeat" :disabled="statucControl[0].next_seat_flg"> {{ trans('events.S_serialSeatTitle') }}
                                            {{-- <input :id="'nextSeat'+item" type="checkbox" @click="seatOnNext(item, $event)" v-bind:checked="settingSeatData[item].seatNextSeat" :disabled="statucControl[0].next_seat_flg"> {{ trans('events.S_serialSeatTitle') }} --}}
                                            <div class="control__indicator"></div>
                                        </label>
                                    </div>
                                    <div class="col-xs-2 buttons text-right btn-safari">
                                        <button type="button" class="btn waves-effect waves-light btn-danger btn-header-right btn-m" @click="removeRowAllSettingSeat(item)" v-show="seatDelBtn(settingSeatData[item])"> 
                                            {{ trans('events.S_seatTypeDeleteBtn') }}
                                        </button>
                                    </div>
                                </div>
                                <!-- /.col -->
                                ...
                                <table class="table table-striped table-ticket">
                                    <thead>
                                        <tr>
                                            ...
                                            <th>{{ trans('events.S_TicketEarlyBirdTitle') }}  </th>
                                            <th>{{ trans('events.S_TicketNormalTitle') }}</th>
                                            <th>{{ trans('events.S_TicketOnSiteTitle') }}</th>
                                            <th class="text-right">
                                                <button class="btn btn-inverse btn-m" @click="insertSettingSeatType(item)" v-show="!statucControl[0].ticket_sales_kbn">{{ trans('events.S_TicketTypeInsertBtn') }}</button>
                                            </th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                            ...
                                            <tr v-show="(infTicket.ticketStatus !== 'D')">
                                                ...
                                                <td>
                                                    ...
                                                    <!-- <div v-bind:class="{ 'select-error-tip': infTicket.ticketNameErrorStatus }"> -->
                                                        ...
                                                        <span v-show="infTicket.ticketNameErrorStatus" class="help is-danger"><i class="fas fa-exclamation-circle"></i> {{ trans('events.S_basicerrorMsn') }}</span>{{-- STS 2021/07/27 Task 38  --}}
                                                    <!-- </div>                                          -->
                                                </td>
                                                <td>
                                                    <!-- <input name="text" type="number" min="0" class="form-control text-right  select-seat-price" v-bind:style="{ borderColor: (infTicket.ticketErrorStatus)?'#e44e2d':'' }"  :value="infTicket.ticketPrice"  @blur="settingInputPrice(item, id, $event)" :disabled="statucControl[0].price" placeholder="{{ trans('events.S_ticketPricePlaceholder') }}"> -->
                                                    <!-- STS 2021/06/12: 小切手 4 つの特殊文字 (+)、(-)、(.)、(,) -->
                                                    <input @paste="validateNumber" onkeypress="return event.charCode >= 48 && event.charCode <= 57" name="text" type="number" min="0" class="form-control text-right  select-seat-price" v-bind:style="{ borderColor: (infTicket.ticketErrorStatus)?'#e44e2d':'' }"  :value="infTicket.ticketPrice"  @blur="settingInputPrice(item, id, $event)" :disabled="statucControl[0].price" placeholder="{{ trans('events.S_ticketPricePlaceholder') }}">                                                
                                                </td>
                                                <td>
                                                    <label class="control control--checkbox">
        ...
        <div class="row" v-show="(typeTicketSetting == 'selectSeat') ? true : false">
            <div class="offset-pl-5 col-xs-12 mb-7">
                <button id="insertSelectSeat" class="btn btn-block waves-effect waves-light btn-rounded btn-inverse" @click="insertSelectSeat()" v-show="!statucControl[0].ticket_sales_kbn">
                    {{ trans('events.S_seatTypeInsertBtn') }}
                </button>
            </div>
        </div>
        <!-- /.BTN + L -->
        ...
        <div class="" v-show="(typeTicketSetting == 'selectSeat') ? true : false">
            ...
            <div class="callout callout-info">
                <!-- -->
                <h4>{{ trans('events.S_reserveSeatSettingTitle') }}</h4>
                {!! trans('events.S_reserveSeatSettingDesc') !!}
            </div>
            </div>
        </div>
        <!--/.提示訊息 -->
                ...
                <div class="box-header with-border-non flex-start">
                    ...
                    <div class="box-tools pull-right">
                        <button  id="insertSpecSeat" type="button" class="btn waves-effect waves-light btn-inverse btn-m" v-on:click="insertSpecSeat()" v-show="!statucControl[0].basis">   
                            {{ trans('events.S_reserveSeatInsertBtn') }}
                        </button>
                    </div>
                    <div class="col-xs-4 pl-50">
                        <h3 class="box-title"> {{ trans('events.S_reserveNameTitle') }} </h3>
                    </div>
                    <div class="col-xs-8 pl-40">
                        <h3 class="box-title"> {{ trans('events.S_reserveSymbol') }} <small>{{ trans('events.S_reserveShortNameDesc') }}</small> </h3>
                    </div>
                    <!-- /.0618調整 -->
                </div>
                            ...
                            <div class="col-xs-12 ptb-8x form-group-flex">
                                <div class="col-xs-4 pl-x has-feedback">
                                    <div v-bind:class="{ 'select-error-tip': item.ticketNameErrorStatus }"> 
                                        <select :id="'ssd-select-'+index" class="form-control select2 select-create" style="width: 100%;" :value="item['ticketName']" :data-index="index" onchange="ticketSetting.specSeatSelectChange(this)" :disabled="statucControl[0].reserve_name">
                                            <option value="" class="not-select" selected="true" disabled="disabled">{{ trans('events.S_reserveSeatPlaceholder') }}</option> 
                                ...
                                <div class="col-xs-6 pr-x buttons text-right">
                                    <button type="button" class="btn waves-effect waves-light btn-danger btn-header-right btn-m" v-on:click="removeSpecSeatRow(index)" v-show="item['ticketStatus'] == 'I' || !statucControl[0].ticket_del_kbn"> 
                                        {{ trans('events.S_reserveseatDeleteBtn') }} 
                                    </button>
                                </div>
                            </div>
                            <!-- /.col -->
...
<script>
const IS_SETTING_TICKET = @json($eventData["is_setting_tikcet"]);
var ticketSetting = new Vue({
    ...
    methods: {
        ...
        initColorPick:function(id, color){
            $.getScript("{{ asset('js/colorPick.js') }}", function(){
                $(id).colorPick({
                    'initialColor': color,
                    'allowRecent': true,
                    'recentMax': 5,
        ...
        */
        cheackReserveSeatData:function(){
            ...
            let errorMsg            =  "{{ trans('events.S_ErrMsgeTitle') }}"
            let status              = false
            ...
            for(let n=0; n<reserveSeatTotal; n++){
                ...
                if(reserveSeat[n].ticketName.trim().length > 0 || reserveSeat[n].ticketStatus == 'D'){
                    ...
                    reserveSeat[n].ticketErrorMsn = "{{ trans('events.S_ErrMsge_06') }}"
            ...
            if(errorStattus){
                errorMsg && (errorMsg += "、");
                errorMsg += "{{ trans('events.S_ErrMsge_06') }}" + `：{{ trans('events.S_ErrNo') }}(${errorNum})` 
            }
            ...
            for(let n=0; n<reserveSeatTotal; n++){
                ...
                if(reserveSeat[n].ticketText.trim().length > 0 || reserveSeat[n].ticketStatus == 'D'){
                    ...
                    reserveSeat[n].ticketErrorMsn    = "{{ trans('events.S_ErrMsge_04') }}"
            ...
            if(errorStattus){
                errorMsg && (errorMsg += "、");
                errorMsg += "{{ trans('events.S_ErrMsge_04') }}" + `：{{ trans('events.S_ErrNo') }}(${errorNum})` 
            }
            ...
            for(let n=0; n<reserveSeatTotal; n++){
                for(let m=n; m<reserveSeatTotal; m++){
                    if(
                        ...
                        reserveSeat[n].ticketErrorMsn = "{{ trans('events.S_ErrMsge_05') }}"
                        reserveSeat[n].ticketErrorStatus = true
                        reserveSeat[m].ticketErrorMsn = "{{ trans('events.S_ErrMsge_05') }}"
            ...
            if(errorStattus){
                errorMsg && (errorMsg += "、");
                errorMsg += "{{ trans('events.S_ErrMsge_05') }}"
            }
        ...
        */ 
        cheackFreeSeatData:function(){
            ...
            if(this.freeSeatTicketName.trim().length === 0){
                errorMsg += "{{ trans('events.S_ErrMsge_07') }}"
            ...
            for(let n=0; n<parentCol; n++){
                ...
                // else{
                //     this.freeSeatData[n].ticketErrorMsn = "{{ trans('events.S_ErrMsge_01') }}"
            ...
            if(errorStattus){
                errorMsg += `{{ trans('events.S_ErrMsge_01') }}：{{ trans('events.S_ErrNo') }}(${errorNum})` 
            }

            if(sejErrorStattus){
                errorStattus && (errorMsg += "、");
                errorMsg += `券種名は半角文字・半角スペースを21文字以上続けないでください。：{{ trans('events.S_ErrNo') }}(${sejErrorNum})` 
            }
            ...
            //     }else{
            //         this.freeSeatData[n].ticketErrorMsn = "{{ trans('events.S_ErrMsge_02') }}"
            ...
            if(errorStattus){
                errorMsg && (errorMsg += "、");
                errorMsg += "{{ trans('events.S_ErrMsge_02') }}" + `：{{ trans('events.S_ErrNo') }}(${errorNum})`
            }
            ...
            for(let n=0; n<parentCol; n++){
                for(let m=n; m<parentCol; m++){
                    ...
                    // STS 2021/06/09 : task 10 parseInt ticketPrice END
                        ...
                        this.freeSeatData[n].ticketErrorMsn = "{{ trans('events.S_ErrMsge_03') }}"
                        this.freeSeatData[n].ticketErrorStatus = true
                        this.freeSeatData[m].ticketErrorMsn = "{{ trans('events.S_ErrMsge_03') }}"
            ...
            if(errorStattus){
                errorMsg && (errorMsg += "、");
                errorMsg += "{{ trans('events.S_ErrMsge_03') }}"
            }
        ...
        settingInputPriceCheack:function(id){
            ...
            if(this.settingSeatData[id].seatStatus != 'D'){
                ...
                if(this.settingSeatData[id].seatName.trim().length === 0){
                    this.settingSeatData[id].seatErrorShowMsn =  "{{ trans('events.S_ErrMsge_07') }}"
                ...
                for(let n=0; n<parentCol; n++){
                    ...
                    // else{
                        // if (!checkMsg) {
                        //     seatData[n].ticketErrorMsn = "{{ trans('events.S_ErrMsge_01') }}"
                ...
                //     }else{
                //         seatData[n].ticketErrorMsn = "{{ trans('events.S_ErrMsge_02') }}"
    ...
    mounted(){
            @if($eventData["status"] === 'edit' || count($errors) > 0)
                sessionStorage.setItem('ticketSetting','{!! addslashes($eventData["ticketSetting"]) !!}')
                sessionStorage.setItem('specTicketSetting','{!! addslashes($eventData["specTicketSetting"]) !!}')
                sessionStorage.setItem('ticketOption','{!! addslashes($eventData["ticketOption"]) !!}')
                let id = this.settingSeatData.length + 1
