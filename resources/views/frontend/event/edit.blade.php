...
@if( $eventData["status"] === 'edit') 
    <h1>
        {{ trans('events.S_mainTitle_01') }}
        {{-- <small>{{ trans('events.S_mainTitle_04') }}</small> --}}
    <!-- <small class="edit-status">
        </small>-->
    </h1>
    <!-- 網站導覽 -->
    <ol class="breadcrumb">
        <li><a href="/events" onclick="loading.openLoading()">{{ trans('events.S_mainTitle_02') }}</a></li>
        <li class="active">{{ trans('events.S_mainTitle_04') }}</li>
    </ol>
@else
    <h1>
        {{ trans('events.S_mainTitle_01') }}
        {{-- <small>{{ trans('events.S_mainTitle_03') }}</small> --}}
    </h1>
    <!-- 網站導覽 -->
    <ol class="breadcrumb">
        <li><a href="/events">{{ trans('events.S_mainTitle_02') }}</a></li>
        <li class="active">{{ trans('events.S_mainTitle_03') }}</li>
    </ol>    
@endif

@if( $eventData["status"] === 'edit')
    <form id="settingSend" method="POST" style="visibility: collapse;display: table;" action="/events/info/{{ $eventData['performanceId'] }}" enctype="multipart/form-data">
...
@stop
@section('content_header_setting')
    @component('components/remind', ['remind_code' => $eventData['remind_code']])
    @endcomponent
    <!-- 固定功能按鈕 ｜ 匯出，更新，新規登錄，申請 放置區 -->
    <!-- 0410新增調整 -->
    <div id="FinalSetting" class="funtion-btn-block"> 
        ...
        <div class="switch">
            ...
            <label for="sale-type-nosale" class="switch-label switch-label-off">{{ trans('events.S_eventSaleTypeInformation') }}</label>
            ...
            <label for="sale-type-sale" class="switch-label switch-label-on">{{ trans('events.S_eventSaleTypeSale') }}</label>
            <span class="switch-selection"></span>
        </div>
        <!-- 0917調整 -->
        <small class="edit-status">
        <!--已經發布活動狀態 ：--> 
        <span class="text-gray-light">{{ $CommonPresenter->getStatusString($eventData['performanceDispStatus'], $eventData['sale_type']) }}
        @if(config('app.debug') == true)
            ({{$eventData['performanceDispStatus']}})
        @endif
        </span>
        @if($eventData["transFlg"])
        <span class="text-gray-light font-600 row-5"> ⇢ </span>  
        <!--現編輯活動狀態 ：-->
        <span class="font-600">{{ $CommonPresenter->getStatusString($eventData['draft_dispaly_status'], $eventData['sale_type']) }}
            @if(config('app.debug') == true)
                ({{$eventData['draft_dispaly_status']}})
            @endif
        @endif
        </span>
<span class="font-600 row-5">
       ｜ @if( $eventData["status"] === 'edit' && $eventData['performanceDispStatus'] >= config('constant.performance_disp_status.browse'))
           @if($eventData['edit_status'] == config('constant.edit_status.not') )
                {{trans('common.S_EditStatus_not')}}
            @elseif ($eventData['edit_status']  == config('constant.edit_status.going'))
                {{trans('common.S_EditStatus_going')}}
            @elseif ($eventData['edit_status']  == config('constant.edit_status.complete'))
    ...
    //是否有單場設定
    const MapHadStageSeat = '{{ isset($eventData["map_had_stage_seat"])?$eventData["map_had_stage_seat"]:false }}'
    //販賣資料
    const SaleInfo = @json($eventData["sale_info"] )
...
<div class="content-navonly">	
    <!--<small class="edit-status">
        {{ $CommonPresenter->getStatusString($eventData['performanceDispStatus'], $eventData['sale_type']) }}
    ...
    <ul id='tagControl' class="nav nav-tabs nav-tabs-basic">
        <li  class="active" v-show="basisTag">
            <a id="basisInfPage" href="#basis" data-toggle="tab"><span>{{ trans('events.S_eventInfoTab_01') }}</span></a>
            <!-- 提示效果 - 錯誤訊息 -->
        ...
        <li v-show="timeCourseTag">
            <a id="timePage" href="#date" data-toggle="tab" @click="timePageSetting"><span>{{ trans('events.S_eventInfoTab_02') }}</span></a>
            <!-- 提示效果 - 錯誤訊息 -->
        ...
        <li v-show="ticketTag">
            <a id="ticketSetPage" href="#ticket" data-toggle="tab" @click="ticketsettingSeat"><span>{{ trans('events.S_eventInfoTab_03') }}</span></a>
            <!-- 提示效果 - 錯誤訊息 -->
        ...
        <li v-show="sellSettingTag">
            <a href="#sell" data-toggle="tab"><span>{{ trans('events.S_eventInfoTab_04') }}</span></a>
            <!-- 提示效果 - 錯誤訊息 -->
        ...
        <li v-show="seatSettingTag">
            <a href="#seat" data-toggle="tab" @click="settingSeat"><span>{{ trans('events.S_eventInfoTab_05') }}</span></a>
            <!-- 提示效果 - 錯誤訊息 -->
        ...
        <li v-show="ticketViewTag">
            <a id="ticketContent" href="#other" data-toggle="tab" @click="settingticketView"><span>{{ trans('events.S_eventInfoTab_06') }}</span></a>
            <!-- 提示效果 - 錯誤訊息 -->
    ...
    <div class="row tab-content tab-basic-content">
        <div class="active tab-pane content-width" id="basis">
            @include('frontend.event.editLayout.basic')
        </div>
        <div class="tab-pane content-width time-setting-block" id="date">
            @include('frontend.event.editLayout.timeCourse')
        </div>
        <div class="tab-pane content-width" id="ticket">
            @include('frontend.event.editLayout.ticketSetting')
        </div>
        <div class="tab-pane content-width" id="sell">
            @include('frontend.event.editLayout.sellSetting')
        </div>
        <div class="tab-pane" id="seat">
            @include('frontend.event.editLayout.seatSetting')
        </div>
        <div class="tab-pane content-width" id="other">
            @include('frontend.event.editLayout.other')
        </div>
    </div>
  </div>
  <script>
       ...
       var tagControl = new Vue({
        ...
        mounted(){
            ...
            @if (count($errors) > 0) 
                ...
                let errorJson = '{!! addslashes($errors->first()) !!}'
            ...
            @else
                @if( $eventData["status"] === 'edit')   
                    ...
                    @if($eventData["has_msn"]) 

                        let msnJson = '{!! addslashes($eventData["msn_json"]) !!}'
              ...
              if(result){
                ...
                //   document.getElementById('ticketViewLogo').files = document.getElementById('mobapassLogo').files
                  ...
                  json.push({
                      ...
                      fbgCtrl: '{!! $eventData["fbgCtrl"] !!}',
                      entry_time : '{{ $eventData['entry_time'] }}'
                  })
                  ...
                  if($(event.target).attr('id')=='updateBtn'){
                    ...
                    loading.openLoading()
                    $('#settingSend').attr('action', '/events/info/{{ $eventData['performanceId'] }}');
                    document.getElementById("settingSend").target = "";
                    document.getElementById("settingSend").submit();
                  } else if($(event.target).attr('id')=='preview'){
                    $('#settingSend').attr('action', '/events/preview/{{ $eventData['performanceId'] }}');
                    window.open('/events/preview/{{ $eventData['performanceId'] }}', 'preview','width=1200,height=800,scrollbars=yes');
    ...
    (function($) {
      ...
      $('#keywords').tagsInput({
        width:'auto',
        defaultText: '{{trans("events.S_add")}}',
    ...
    const _colorList = [{!! \Config::get('constant.color') !!}]
    const _textColorList = [{!! \Config::get('constant.color_text') !!}]
    ...
    $('.dropifymbps').dropify({
      ...
      error: {
        'fileSize': '{{trans("common.S_DropifySizeErrMbps")}}'
        }
    });
*/
